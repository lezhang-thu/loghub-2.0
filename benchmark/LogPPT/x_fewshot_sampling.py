...
if __name__ == '__main__':
    os.makedirs("datasets-wo-virtual-label", exist_ok=True)
    for dataset in datasets:
        print(dataset)
        setting = benchmark[dataset]
        os.makedirs("datasets-wo-virtual-label/{0}".format(dataset), exist_ok=True)

        logdf = log_to_dataframe(f'../../2k_dataset/{setting["log_file"]}', setting['log_format'])
        logdf.to_csv(f"datasets-wo-virtual-label/{setting['log_file']}_structured.csv")
        ...
        for shot in [32]:
            keywords_list = []
            os.makedirs("datasets-wo-virtual-label/{0}/{1}shot".format(dataset, shot), exist_ok=True)
            ...
            with open("datasets-wo-virtual-label/{0}/{1}shot/{2}.json".format(dataset, shot, 1), "w") as f:
